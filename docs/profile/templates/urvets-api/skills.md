# 🛠️ urvets-api Skills Installer List

This file contains the list of skills to install for the `urvets-api` profile. Each non-comment line is run as a CLI command inside the container.


# NestJS Best Practices
npx skills add https://github.com/kadajett/agent-nestjs-skills --skill nestjs-best-practices

# NestJS Patterns
npx skills add https://github.com/affaan-m/everything-claude-code --skill nestjs-patterns

# NestJS Expert
npx skills add https://github.com/jeffallan/claude-skills --skill nestjs-expert

# Sentry NestJS SDK
npx skills add https://github.com/getsentry/sentry-for-ai --skill sentry-nestjs-sdk

# NestJS Code Review
npx skills add https://github.com/giuseppe-trisciuoglio/developer-kit --skill nestjs-code-review

# Prisma CLI
npx skills add https://github.com/prisma/skills --skill prisma-cli

# Swagger Doc Creator
npx skills add https://github.com/jeremylongshore/claude-code-plugins-plus-skills --skill swagger-doc-creator

# Brainstorming
npx skills add https://github.com/obra/superpowers --skill brainstorming

# Prisma Database Setup
npx skills add https://github.com/prisma/skills --skill prisma-database-setup

# Database Migration
npx skills add https://github.com/wshobson/agents --skill database-migration

# Database Schema Designer
npx skills add https://github.com/softaworks/agent-toolkit --skill database-schema-designer
