...
async def _is_gateway_running_and_runtime() -> tuple[bool, dict | None]:
    ...
    if not gateway_running and _GATEWAY_HEALTH_URL:
        ...
        if alive:
            ...
            if runtime is None and remote_health_body and remote_health_body.get("gateway_state"):
                runtime = remote_health_body
    return gateway_running, runtime
...
def _messaging_platform_payload(
    entry: dict[str, Any], env_on_disk: dict[str, str], runtime: dict | None, gateway_running: bool | None = None
) -> dict[str, Any]:
...
async def get_messaging_platforms():
    env_on_disk = load_env()
    gateway_running, runtime = await _is_gateway_running_and_runtime()
    return {
        "platforms": [
            _messaging_platform_payload(entry, env_on_disk, runtime, gateway_running=gateway_running)
            for entry in _messaging_platform_catalog()
        ]
    }
...
async def test_messaging_platform(platform_id: str):
    ...
    env_on_disk = load_env()
    gateway_running, runtime = await _is_gateway_running_and_runtime()
    payload = _messaging_platform_payload(entry, env_on_disk, runtime, gateway_running=gateway_running)
